...
export default function ClientDashboard() {
  ...
  const [lastFeedbackDates, setLastFeedbackDates] = useState<Record<string, string>>({});
  ...
  const fetchData = async () => {
    try {
      ...
      if (feedbackResponse.ok) {
        ...
        // Calculate last feedback dates per project
        const dates: Record<string, string> = {};
        feedbackData.data?.forEach((fb: Feedback) => {
          if (!dates[fb.projectId._id] || new Date(fb.createdAt) > new Date(dates[fb.projectId._id])) {
            dates[fb.projectId._id] = fb.createdAt;
          }
        });
        setLastFeedbackDates(dates);
      }
                    ...
                    <div className="flex-1 space-y-4">
                      ...
                      <div className="space-y-2 text-sm text-gray-600">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span>Last feedback: {lastFeedbackDates[project._id] 
                            ? new Date(lastFeedbackDates[project._id]).toLocaleDateString() 
                            : 'Never'}</span>
                        </div>
                      </div>
                    </div>
